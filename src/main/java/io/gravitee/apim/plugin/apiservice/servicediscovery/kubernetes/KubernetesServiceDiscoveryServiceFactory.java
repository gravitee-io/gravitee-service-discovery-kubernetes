...
@AllArgsConstructor
public class KubernetesServiceDiscoveryServiceFactory
  implements ApiServiceFactory<KubernetesServiceDiscoveryService> {

  @Override
  public KubernetesServiceDiscoveryService createService(
    ...
    if (!kubernetesEnabledGroups.isEmpty()) {
      return new KubernetesServiceDiscoveryService(
        deploymentContext,
        kubernetesEnabledGroups
      );
    }
  ...
  private List<
    io.gravitee.definition.model.v4.endpointgroup.EndpointGroup
  > getKubernetesEnabledGroups(Api api) {
    return api
      .getDefinition()
      .getEndpointGroups()
      .stream()
      .filter(group ->
        KubernetesServiceDiscoveryChecker.isKubernetesEnabled(
          group.getServices()
        )
      )
      .collect(toList());
