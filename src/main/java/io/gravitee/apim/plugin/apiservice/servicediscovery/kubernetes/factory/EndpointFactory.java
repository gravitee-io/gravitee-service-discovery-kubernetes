...
public class EndpointFactory {
  ...
  private static final Map<String, EndpointConfigurationFactory> factories =
    new HashMap<>();
  ...
  public static Endpoint build(
    ...
    if (!ENDPOINT_TYPE_HTTP_PROXY.equals(type)) {
      throw new IllegalStateException(
        String.format("The endpoint type [%s] is not supported", type)
      );
    }
    ...
    endpoint.setConfiguration(
      getFactory(type).buildConfiguration(address, port, configuration)
    );
  ...
  private static EndpointConfigurationFactory getFactory(String type) {
    return factories.computeIfAbsent(type, t ->
      new HttpProxyEndpointConfigurationFactory()
    );
  }
